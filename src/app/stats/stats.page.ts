...
interface BucketData {
  label: string; total: number; classMins: number; sportMins: number; extraMins: number;
}

const C = {
  primary: '#c2185b', sport: '#9c27b0', extra: '#e91e63', cours: '#4a90d9',
  gridLine: '#1e1e1e', tick: '#666666', tooltipBg: '#1a1a1a', tooltipBorder: '#333333',
};

// ─── Cache localStorage (partagé avec timer.page.ts) ─────────────────────
const CACHE_KEY = 'nexus_session_type_cache';

function readTypeCache(): Record<number, ActivityType> {
  try { return JSON.parse(localStorage.getItem(CACHE_KEY) ?? '{}'); } catch { return {}; }
}

/**
 * Détecte le type d'activité depuis les champs discriminants C# renvoyés
 * dans activities[] :
 *  - subject / teacher  → class
 *  - intensity          → sport
 *  - organiser / theme  → extra
 */
function detectTypeFromActivity(a: any): ActivityType | null {
  if (!a) return null;
  if (a.subject != null || a.teacher != null) return 'class';
  if (a.intensity != null)                    return 'sport';
  if (a.organiser != null || a.theme != null) return 'extra';
  // room seul → probablement une Class
  if (a.room != null)                         return 'class';
  return null;
}
...
export class StatsPage implements OnInit, OnDestroy {
  ...
  get periodLabel() {
    return ({ jour: 'de la journée', semaine: 'de la semaine', mois: 'du mois', annee: "de l'année" })[this.selectedPeriod];
  }
  ...
  private loadAllData() {
    ...
    this.nexusService.getSessions().pipe(catchError(() => of([]))).subscribe({
      next: (rawSessions: any[]) => {
        this.sessions = rawSessions
          .filter((s: any) => s.dateTimeStart)
          .map((s: any) => this.enrich(s as GetSessionDto));
  ...
  // ENRICHISSEMENT
  //
  // Ordre de priorité pour déterminer le type d'activité :
  //  1. Cache localStorage (écrit par timer.page.ts au moment de la création)
  //  2. Champs discriminants dans activities[0] (subject/teacher/intensity/organiser/theme)
  //  3. 'class' par défaut
  // ─────────────────────────────────────────────────────────────────────────
  private enrich(s: GetSessionDto): ES {
    ...
    // ── 1. Cache localStorage (source la plus fiable) ──────────────────────
    let type: ActivityType = cache[s.id] ?? null;

    // ── 2. Champs discriminants dans activities[] ───────────────────────────
    if (!type && Array.isArray(s.activities) && s.activities.length > 0) {
      type = detectTypeFromActivity(s.activities[0]) ?? 'class';
    }
    ...
    // ── Label & intensity ──────────────────────────────────────────────────
    const firstActivity = Array.isArray(s.activities) ? s.activities[0] : null;
    const label         = firstActivity?.name || s.status || 'Session';
    const intensity     = firstActivity?.intensity ?? null;
  ...
  // ACTIONS UI
  // ─────────────────────────────────────────────────────────────────────────
  selectPeriod(p: Period) { this.selectedPeriod = p; this.compute(); }
  ...
  private getFilteredBuckets(): BucketData[] {
    return this.buildBuckets(this.filterByPeriod(this.sessions, this.selectedPeriod));
  }
  ...
  private buildBuckets(ss: ES[]): BucketData[] {
    ...
    return Array.from({ length: 12 }, (_, i) => {
      return make(MONTHS[i], ss.filter(s => s.dateTimeStart.getMonth() === i));
    });
  }
  ...
  private computeBars(ss: ES[]) {
    const groups = [
      { type: 'class' as const, icon: 'book-outline',    color: C.primary, label: 'Cours' },
      { type: 'sport' as const, icon: 'fitness-outline', color: C.sport,   label: 'Sport' },
      { type: 'extra' as const, icon: 'rocket-outline',  color: C.extra,   label: 'Extra' },
    ];
  ...
  private commonTooltip(labelFmt: (ctx: any) => string) {
    return {
      ...
      titleColor: '#ffffff', titleFont: { size: 12 },
      bodyColor: '#aaaaaa', bodyFont: { size: 11 }, padding: 10,
    ...
    this.investChart = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: this.commonTooltip(c => ` ${c.dataset.label}: ${c.parsed.y.toFixed(1)}h`)
        },
  ...
  private buildHistoChart(buckets: BucketData[], ss: ES[]) {
    ...
    const normData: number[] = [], elevData: number[] = [], highData: number[] = [];
    ...
    this.histoChart = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          legend:  { display: false },
          tooltip: this.commonTooltip(c => ` ${c.dataset.label}: ${c.parsed.y} min`)
        },
        scales: {
